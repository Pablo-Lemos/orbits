...
class LearnForces(tf.keras.Model):
    def __init__(self, nplanets, senders, receivers, norm_layer, noise_level = 0., masses = None):
        ...
        if masses is None:
            ...
            logm_init = tf.constant_initializer(np.log10(masses + 1e-30)+10)
        ...
        self.graph_network_newtonian = gn.blocks.EdgeBlock(
            edge_model_fn=lambda: tf.keras.layers.Dense(3, use_bias = False,
                                             kernel_initializer=M,
                                             trainable = False
                                                        ),
            use_edges = True,
        ...
        self.graph_network_pert = gn.blocks.EdgeBlock(
            ...
            edge_model_fn=lambda: snt.Sequential([
                #norm_layer,
                tf.keras.layers.Dense(5, input_dim=5,
    ...
    def call(self, D, training = False, extract = False):
        ...
        lm = tf.clip_by_value(lm, -20, 12, name=None)
        self.logmasses = lm
        ...
        g = self.graph_network_newtonian(g)
        g = self.graph_network_pert(g)
        g = g.replace(
    ...
    def flatten_weights(self):
        ...
        self.numWeights = len(flat)
        #for layer in self.graph_network.trainable_variables:
        #    flat = tf.concat([flat, tf.reshape(layer, -1)], axis=0)
        return flat
    ...
    def initiate_weights(self, data):
        if self.numWeights is None:
            self.flatten_weights()

        # Unpack the data
        D, A = data.get_single_element()

        # samples = tf.random.normal(
        #     shape=(nLive, self.numWeights),
        #     mean=0.0,
        #     stddev=3.0)
        #
        # likes = []
        # for i in range(nLive):
        #     self.load_weights(samples[i])
        #     self.evaluate(data)
        #     predictions = self(D)
        #     likes.append(min(1e30, mean_weighted_error(A, predictions,self.nplanets).numpy()))
        ...
        while acceptance > 0.01:
            newSample = tf.random.normal(
                shape=(self.numWeights, ),
                mean=0.0,
                stddev=0.1)
            self.load_weights(newSample)
            predictions = self(D)
            newLike = min(1e100, mean_weighted_error(A, predictions,self.nplanets).numpy())
            print(bestLike, newLike, acceptance)
            ...
            numTried += 1
            acceptance = numAccepted / numTried

        self.load_weights(bestSample)
