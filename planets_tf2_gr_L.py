...
num_time_steps_tr = 856000  # Number of time steps for training (~27 years). 30y=50400; 40y=680800; 50y=856000
...
def read_data(num_time_steps_tr, num_time_steps_val):
    ...
    filename = './simulator/1000_gr_simulation_1pl_50y.pickle'
    ...
    data = np.concatenate([x, L, a], axis=-1)
...
def format_data_gnn(data_tr, data_val, system):
    ...
    D_L_tr = np.empty([len(data_tr), nedges, 6])
    D_L_val = np.empty([len(data_val), nedges, 6])
    ...
    for i in range(system.numPlanets):
        for j in range(system.numPlanets):
            if i > j:
                ...
                D_L_tr[:, k, :] = data_tr[:, j, :6] - data_tr[:, i, :6]
                D_L_val[:, k, :] = data_val[:, j, :6] - data_val[:, i, :6]
...
def main(system, train_ds, test_ds, norm_layer, senders, receivers):
    ...
    checkpoint_filepath = 'saved_models/sun_mercury_1000_gr_50y_L_2'
