...
class Normalize_gn(tf.keras.layers.Layer):
    def __init__(self, D_V):
        ...
        self.maxs_L = tf.reduce_max(D_V[:, 3:4], axis=0)
        self.mins_L = tf.reduce_min(D_V[:, 3:4], axis=0)
    ...
    def call(self, inputs):
        ...
        maxs = tf.concat([maxs_r, maxs_theta, maxs_phi, maxs_L, maxs_theta, maxs_phi, maxs_m, maxs_m], axis=-1)
        mins = tf.concat([mins_r, mins_theta, mins_phi, mins_L, mins_theta, mins_phi, mins_m, mins_m], axis=-1)
...
class LearnForces(tf.keras.Model):
    ...
    def call(self, D_L, training=False, extract=False):
        # print(f"D_V shape in call: {D_V.shape}")
        # print((f"D_V in call: {D_V}"))
        # print(self.nedges)
        if D_L.shape[0] is None:
            return D_L
        ntime = int(D_L.shape[0] // self.nedges)
        if training == True:
        ...
        if training == True:
            senders_g, receivers_g, signs = shuffle_senders_receivers(senders_g,
                                                                      receivers_g)
            D_L = D_L * signs[:, np.newaxis]

        # Create a graph
        graph_dict_D_V = {
            "nodes": nodes_g,
            "edges": cartesian_to_spherical_coordinates(D_L),
        ...
        graphs_nx = gn.utils_np.graphs_tuple_to_networkxs(g)
    ...
    def train_step(self, data):
        ...
        # Unpack the data
        D_L, A = data
        print(f'D_V shape in training step {D_L.shape}')
        print(f'A shape in training step {A.shape}')

        D_L_rs = tf.reshape(D_L, shape=[-1, self.nedges, 6])
        A_rs = tf.reshape(A, shape=[-1, self.nplanets, 3])
        # Randomly 3D rotate the data
        D_L_rot, A_rot = rotate_data(D_L_rs, A_rs, uniform=False)
        D_L_rot = tf.reshape(D_L_rot, shape=[-1, 6])
        A_rot = tf.reshape(A_rot, shape=[-1, 3])
        # print(f'D_V shape in training step after rotation {D_V_rot.shape}')
        # Add Noise
        D_L_noise = tf.random.normal(tf.shape(D_L_rot), 0, self.noise_level, tf.float32)
        D_L_rot = D_L_rot * (1 + D_L_noise)

        with tf.GradientTape() as tape:
            # Forward pass
            predictions = self(D_L_rot, training=True)
            # Compute the loss
    ...
    def test_step(self, data):
        # Unpack the data
        D_L, A = data
        print(f'D_V shape in test step {D_L.shape}')
        print(f'A shape in test step {A.shape}')

        predictions = self(D_L)
